...
let prefix = ayarlar.prefix
   ...
   if(!kanal) return message.channel.send(
     ...
     .setColor("#501c67")
     .setDescription('<a:r_hayir:785437821435117608> Güvenlik mesajlarının gideceği kanalı etiketlemedin.'))
   else {
    db.set(`güvenlik.${message.guild.id}`, kanal.id)
    return message.channel.send(
      new Discord.MessageEmbed()
      .setColor("#501c67")
      .setDescription('<a:r_evet:785437821926113290> Güvenlik kanalı <#'+kanal+'> olarak ayarlandı.'))
   }
   if(args[0] === 'sıfırla') {
    db.delete(`güvenlik.${message.guild.id}`)
     const sıfırladın = new Discord.MessageEmbed()
     .setColor("#501c67")
     .setDescription('<a:r_evet:785437821926113290> Güvenlik kanalı başarıyla sıfırlandı.')
