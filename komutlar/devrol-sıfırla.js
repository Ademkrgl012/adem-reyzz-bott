...
exports.run = async (client, message, args) => {
  ...
  if (kontrol == "TR_tr") {
    if(!message.member.hasPermission('MANAGE_ROLES')) return message.channel.send('Bu komutu kullanmak için gerekli yetkiye sahip değilsin')    
      if(!db.has(`${message.guild.id}_devrol`)) return message.channel.send('Zaten bot rolü ayarlanmamış'); 
   ...
   if(!message.member.hasPermission('MANAGE_ROLES')) return message.channel.send('You do not have permissions to use this command')    
      if(!db.has(`${message.guild.id}_devrol`)) return message.channel.send('Bot role is not already set'); 
            db.delete(`${message.guild.id}_devrol`)
            message.channel.send('Developer role succesfully has been reset')
}
exports.conf = {
    aliases: ["devrole-reset"]
}
exports.help = {
    name: "devrol-sıfırla"
}